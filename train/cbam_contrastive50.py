...
#learning stuff
model = nn.DataParallel(nets.cbam_classifier_50(freeze_mode='none',drop_classifier=True)).cuda()
...
if load_checkpoint(checkpoint_name=checkpoint_name) == 0:
    print('Loaded starting checkpoint: ' + str(load_checkpoint(checkpoint_name = 'ch_cbam50_classifier',model=model)),flush=True)
    start_index = 0
else:

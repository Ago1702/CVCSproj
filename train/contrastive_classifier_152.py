...
wandb.init(
    ...
    project="Vanilla-Resnet152-Classifier",
    name = "run 1",
...
checkpoint_name = 'ch_cbam152_contrastive_classifier'
...
#learning stuff
model = nn.DataParallel(nets.cbam_classifier_152(freeze_mode='embedder')).cuda()
...
if load_checkpoint(checkpoint_name) == 0:
    start_index = 0
    print('Loaded starting checkpoint number: '+ str(load_checkpoint('ch_cbam152_contrastive',model=model)))
else:

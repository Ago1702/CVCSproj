...
if __name__ == "__main__":
    ...
    if path.exists('/homes/dagostini/CVCSproj/CVCSproj/model/res_weight.pth'):
        print("Loaded from file", flush=True)
        res_net.load_state_dict(torch.load('/homes/dagostini/CVCSproj/CVCSproj/model/res_weight.pth', weights_only=True))
        res_net.eval()
    ...
    for n, (images, labels) in enumerate(dataloader):
        ...
        if((n + 1) % 50 == 0):
            torch.save(res_net.state_dict(), '/homes/dagostini/CVCSproj/CVCSproj/model/res_weight.pth')

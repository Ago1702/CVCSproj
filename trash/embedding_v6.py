...
from models.resnets import v6
import wandb
import time
from utils.helpers import state_dict_adapter

if __name__ == "__main__":
    #good iter 4000 for run good_1
    test_batch_size= 100
    iteration_index = 7000
    ...
    embedder = v6()
    embedder.load_state_dict(state_dict_adapter(torch.load(f'/work/cvcs2024/VisionWise/weights/checkpoint_rescbam_contr_good_r1_{iteration_index}.pth',weights_only=False)['model'],string_to_remove='module.'))
    ...
    plt.savefig(os.path.expanduser(f'~/CVCSproj/outputs/rescbam_contr_good_r1_{iteration_index}.png'))
    plt.close()
